...
describe("a Watchtower's note row", () => {
  ...
  it("claims coverage only while the watcher is inside", () => {
    expect(watchNote({ watching: 3, worker: "inside" })).toBe("watching 3 tiles of shore");
  });
  ...
  it("says the watcher is away while they walk or eat", () => {
    ...
    expect(watchNote({ watching: 3, worker: "walking" })).toBe("3 tiles of shore in reach — the watcher is away");
    expect(watchNote({ watching: 3, worker: "eating" })).toBe("3 tiles of shore in reach — the watcher is away");
  });

  it("says what an unstaffed tower would watch", () => {
    expect(watchNote({ watching: 3, worker: "none" })).toBe("3 tiles of shore in reach — no watcher");
  });

  it("counts one tile of shore in the singular", () => {
    expect(watchNote({ watching: 1, worker: "inside" })).toBe("watching 1 tile of shore");
    expect(watchNote({ watching: 1, worker: "none" })).toBe("1 tile of shore in reach — no watcher");
  });
  ...
  it("puts a tower in the wrong place above every other wording", () => {
    ...
    // facing inland is a tower sited badly, not a staffing problem.
    for (const worker of ["inside", "walking", "eating", "none"] as const) {
      expect(watchNote({ watching: 0, worker })).toBe("the watcher sees no shore from here");
    }
  });
});
